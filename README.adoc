...
* GitHub is owned by Microsoft—the same company that used link:https://en.wikipedia.org/wiki/Embrace,_extend,_and_extinguish[embrace, extend, extinguish ({abbr-EEE})] in the past to ruin software as a whole. Microsoft is trying to further monopolize with Azure, Visual Studio (regular & Code), {abbr-NPM}, and trying to steal mindshare from free software {abbr-WSL}. Over time, they have been pushing for tighter and tighter integrations.
