...
• The *D* in *DCVS* is for distribrited which Git can (& probably should) be which should mean a project does not require a specific platform. Even if you would prefer an alternative :ac:`UX`, there should not be a barrier to contribution beyond email if a user does not wish to share data with :ac:`MS` GitHub (alternatively, decentralized authentication). Due to this, many projects are *stuck* on :ac:`MS` GitHub thru vendor lock-in. As a user, I *must* fork your project on :ab:`MS` GitHub to submit a pull request & I *must* have an account to raise or comment on issues. Worse is some communities, like Elm & Nix have tied their user identity in the community to this closed system.
• :ac:`MS` GitHub is trying to make code unnecessarily ‘social’ which doesn’t need to exist. We don’t need targeted advertising for other projects or anxiety about how colorful status progress bars are. Plug: I have created a `uBlock Origin list <https://git.sr.ht/~toastal/github-less-social>`_ to help declutter your MS GitHub experience while forced to be here.
